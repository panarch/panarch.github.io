...
eval("const gluesql = __webpack_require__.e(/*! import() */ 0).then(__webpack_require__.bind(null, /*! ./node_modules/gluesql/gluesql.js */ \"./node_modules/gluesql/gluesql.js\"));\n\nconst BLUE = '#3090F2';\nconst RED = '#f72145';\n\nconst stringify = v => JSON.stringify(v, null, ' ');\n\nlet db;\n\nasync function init() {\n  document.querySelector('#run').addEventListener('click', (e) => {\n    e.preventDefault();\n\n    let sql = document.querySelector('textarea').value;\n\n    run(sql);\n  });\n\n  const { Glue } = await gluesql;\n\n  const storage = prompt('Choose storage type: memory | localstorage | sessionstorage', 'memory');\n  const namespace = (\n    storage == 'memory' ?\n    null :\n    prompt('Database name?', 'whatever')\n  );\n\n  document.querySelector('#storage-type').textContent = storage;\n  document.querySelector('#database-name').textContent = namespace;\n\n  db = (() => {\n    try {\n      return new Glue(storage, namespace);\n    } catch (e) {\n      print({\n        color: RED,\n        content: `[ERR] ${stringify(e)}`,\n      });\n\n      alert('refresh for re-run');\n    }\n  })();\n}\n\nasync function run(sql) {\n  let message;\n\n  try {\n    let result = db.execute(sql)[0];\n\n    message = {\n      color: BLUE,\n      content: `[RES] ${stringify(result)}`,\n    };\n  } catch (error) {\n    message = {\n      color: RED,\n      content: `[ERR] ${stringify(error)}`,\n    };\n  }\n\n  print(message);\n  print({\n    color: 'white',\n    content: `[RUN] ${sql}`,\n  });\n\n  print(' ');\n  print(' ');\n}\n\nfunction print(message) {\n  const code = document.createElement('code');\n\n  if (message) {\n    const { content, color } = message;\n\n    code.textContent = content;\n    code.style.color = color;\n  }\n\n  document.querySelector('#feed').appendChild(code);\n}\n\ninit();\n\n\n//# sourceURL=webpack:///./index.js?");
